...
def opcaoEscolhida(mnu):
    ...
    return umTexto(
        "Qual é a sua opção? ",
        "Opção inválida",
        [str(opc) for opc in range(1, len(mnu) + 1)],
    )
...
def ondeEsta(nom, agd):
    ...
    return -(
        inicio + 1
    )  # retornamos, negativada, a posicao onde inserir (já que não encontramos o que procurávamos) +1
...
def excluir(agd):
    ...
    resposta = umTexto(
        "Deseja realmente excluir? ", "Você deve digitar S ou N", ["s", "S", "n", "N"]
    )
